...
def print_node(sentence, word, first_word_id, second_word_id, unitword_misc):
	...
	output += (',{{"id":{id_node},"parent":{id_parent},"order":{order},{firstson_str}{rbrother_str}' '"data":{{"ord":{order},"form":"{form}","lemma":"{lemma}","upos":"{upos}",' '"xpos":"{xpos}","feats":"{feats}","deprel":"{deprel}",' '"misc":"{misc}","id":"{address}"}},' '"labels":["{form}","#{{#bb0000}}{upos}","#{{#0000bb}}{deprel}"],' '"hint":"lemma={lemma}\\n{multiline_feats}"}}'.format(**locals())) + '\n'
